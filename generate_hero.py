...
def get_fonts():
    # Large clean modern sans-serif like reference
    name_font = ImageFont.truetype("C:/Windows/Fonts/arial.ttf", 78)
    sub1_font = ImageFont.truetype("C:/Windows/Fonts/arial.ttf", 36)
    sub2_font = ImageFont.truetype("C:/Windows/Fonts/arial.ttf", 32)
...
def render_frame(f_idx):
    ...
    # Reveal timeline for name: Frame 25 to 45
    if f_idx >= 22:
        name_prog = min(1.0, (f_idx - 22) / 16.0)
        
        # Subtle glitch displacement
        is_glitching = (22 <= f_idx <= 32 and f_idx % 2 == 0)
        disp_x = random.randint(-3, 3) if is_glitching else 0
        disp_y = random.randint(-1, 1) if is_glitching else 0

        # Draw Clean Pure White Main Title
        ...
        if 35 <= f_idx <= 60:
            sweep_p = (f_idx - 35) / 25.0
            sweep_x = name_x - 40 + int((name_w + 80) * sweep_p)
            draw.line([(sweep_x - 10, name_y - 5), (sweep_x + 10, name_y + name_h + 10)], 
                      fill=(255, 255, 255, int(160 * math.sin(sweep_p * math.pi))), width=2)
    ...
    sub1_y = name_y + name_h + 30

    if f_idx >= 38:
        sub1_prog = min(1.0, (f_idx - 38) / 14.0)
    ...
    sub2_y = sub1_y + sub1_h + 20

    if f_idx >= 48:
        sub2_prog = min(1.0, (f_idx - 48) / 14.0)
